...
patientRoute.post("/login", async (req, res) => {
    ...
    try {
        bcrypt.compare(password, data.password, function (err, result) {
            if (result) {
                var token = jwt.sign({ patientID: data._id }, process.env.key, { expiresIn: "1h" }); // 1-hour expiry
                var refreshtoken = jwt.sign({ patientID: data._id }, process.env.key, { expiresIn: "7d" }); // 7-day expiry
