...
adminRoute.post("/login", async (req, res) => {
    ...
    try {
        bcrypt.compare(password, data.password, function (err, result) {
            if (result) {
                var token = jwt.sign({ adminID: data._id }, process.env.key, { expiresIn: "1h" }); // Access Token (expires in 1 hour)
                var refreshtoken = jwt.sign({ adminID: data._id }, process.env.key, { expiresIn: "7d" }); // Refresh Token (expires in 7 days)
