...
doctorRoute.post("/login", async (req, res) => {
    ...
    try {
        bcrypt.compare(password, data.password, function (err, result) {
            if (result) {
                var token = jwt.sign({ doctorID: data._id }, process.env.key, { expiresIn: "1h" }); // 1-hour expiry for access token
                var refreshtoken = jwt.sign({ doctorID: data._id }, process.env.key, { expiresIn: "7d" }); // 7-day expiry for refresh token
